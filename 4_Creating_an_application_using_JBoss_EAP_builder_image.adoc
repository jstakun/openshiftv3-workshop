...
In the search text box type `eap cd` and select the `JBoss EAP CD (no https)`. EAP CD is latest EAP 7 version updated more frequently than regular EAP 7.x versions.

Click *Next* button and fill the fields with the following information:
...
MAVEN_MIRROR_URL: http://nexus.devtools.svc:8081/repository/maven-all-public/
....

Click on *Next* and *Create* buttons and then *Continue to overview* link.
...
ks        ks-myjbossapp-userXX.<<RouteDomainSuffix>>                                  ks     8080-tcp   
....
...
....
$ curl ks-myjbossapp-userXX.<<RouteDomainSuffix>>
...
Go to https://ks-myjbossapp-userXX.<<RouteDomainSuffix>> via your browser.
...
Go to your EAP pod log at check what is value of -Xms -Xmx jvm parameters in the very beginning of log

Add to your EAP deployment config environment variable Name: CONTAINER_HEAP_PERCENT Value: 0.9

Wait for EAP pod restart and check again value of -Xms -Xmx jvm parameters
