...
* You will be using the projects you created in lab 12 "Code Promotion Across Applications".

*Step 1:* Create a new project Create a new project with name
`pipeline-userXY`. _Substitute userXY with your own user name._
...
`https://jenkins-pipeline-userXY.apps.devday.osecloud.com`
...
openshiftBuild(namespace: 'development-userXY', buildConfig: 'welcome-php', showBuildLogs: 'true')
stage 'deployInDevelopment'
openshiftDeploy(namespace: 'development-userXY', deploymentConfig: 'welcome-php')
openshiftScale(namespace: 'development-userXY', deploymentConfig: 'welcome-php',replicaCount: '2')
stage 'deployInTesting'
input 'Promote Dev Image to Testing?'
openshiftTag(namespace: 'development-userXY', sourceStream: 'welcome-php',  sourceTag: 'latest', destinationStream: 'welcome-php', destinationTag: 'promote-qa')
openshiftDeploy(namespace: 'testing-userXY', deploymentConfig: 'welcome-php', )
openshiftScale(namespace: 'testing-userXY', deploymentConfig: 'welcome-php',replicaCount: '3')
}
....
...
to have access to the projects `development-userXY` and
`testing-userXY`. Just for testing; we will give jenkins
full access to these projects.

....
oc project jenkins-userXY
oc adm policy add-role-to-user admin system:serviceaccount:jenkins-userXY:jenkins -n testing-userXY
oc adm policy add-role-to-user admin system:serviceaccount:jenkins-userXY:jenkins -n development-userXY
....

For reference the syntax is: `system:serviceaccount:PROJECTNAME:SERVICEACCOUNT`

Next, we will edit the `development-userXY` project to disable
...
Do the same steps for the `testing-userXY` project. It should 
look similar.
