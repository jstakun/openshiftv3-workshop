...
new project with name `spring-userXY`. *Remember* to substitute the
`userXY` with your userid.
...
....
$ oc new-app redhat-openjdk18-openshift:1.5~https://github.com/RedHatWorkshops/spring-sample-app --name=bootapp --build-env=MAVEN_MIRROR_URL=http://nexus.devtools.svc:8081/repository/maven-all-public/
--> Found Docker image c3ddd9e (7 days old) from Docker Hub for "veermuchandi/spring-mvn-base"
    ...
    * An image stream will be created as "redhat-openjdk18-openshift:1.5" that will track the source image
...
bootapp   bootapp-spring-userXY.apps.devday.ocpcloud.com             bootapp    8080-tcp   
....
...
In SpringBoot pod log check what is value of -Xms -Xmx jvm parameters

Add to SpringBoot deployment config environment variable Name: JAVA_MAX_MEM_RATIO Value: 90

Wait for SpringBoot pod restart and check again value of -Xms -Xmx jvm parameters
